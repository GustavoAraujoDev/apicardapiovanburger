class Pedido {
  constructor({
    ...
    if (!cliente || !cliente.nome || !cliente.telefone) {
      throw new Error(
        "Informações do cliente são obrigatórias (nome e telefone)."
      );
    }
    ...
    itens.forEach((item) => {
      if (!item.name || !item.category || !item.Quantity || !item.price) {
        throw new Error(
          "Todos os itens devem conter nome, categoria, quantidade e preço."
        );
      }
    });
    ...
    if (
      !modoEntrega ||
      !["entrega em casa", "consumo no local"].includes(modoEntrega)
    ) {
  ...
  formatarParaImpressao() {
    try {
      ...
      texto += `Endereço: ${this.cliente.endereco}\n`;
      texto += "Itens:\n";
      this.itens.forEach((item) => {
        texto += `${item.Quantity} x ${item.name} (${item.category}) ${
          item.description
        } ${item.observacao} - R$ ${item.price.toFixed(2)}\n`;
      });
      ...
      if (this.modoEntrega === "entrega em casa") {
        ...
        texto += `Rastreamento: ${this.rastreamento.statusRastreamento} (${
          this.rastreamento.codigoRastreamento || "N/A"
        })\n`;
